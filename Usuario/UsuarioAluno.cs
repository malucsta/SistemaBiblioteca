    ...
    public abstract class UsuarioAluno : Usuario
    {
        ...
        public override string Emprestar(Livro livro)
        {
            ...
            if (!livro.PodeSerEmprestado())
            {
                ...
                {
                    var emprestimoRealizado = EmprestarLivro(livro);
            ...
            if (Emprestimos.Any(emprestimo => emprestimo.CodigoLivro == livro.Codigo && emprestimo.Status == EmprestimoStatus.Ativo))
                return baseMotivo + $"Já existe um empréstimo em andamento para o livro {livro.Codigo}";

            var emprestimo = EmprestarLivro(livro);
